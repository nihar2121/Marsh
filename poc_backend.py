...
def process_united_india_insurance(file_path, template_data, risk_code_data, cust_neft_data, table_3, table_4, table_5, subject, mappings):
    try:
        ...
        for i in range(len(header_indices) - 1):
            ...
            # Extract and clean header row: ensure all headers are strings
            header_row = [str(col).strip() for col in data.iloc[start_idx].values]
            df_section = data.iloc[start_idx + 1:end_idx].reset_index(drop=True)

            # Remove empty rows in df_section
            df_section = df_section.dropna(how='all').reset_index(drop=True)

            # Assign cleaned header row as column names
            df_section.columns = header_row
        ...
        for idx, df in enumerate(dataframes):
            ...
            # Format 'Policy End Date' and 'Policy Start Date' into 'dd/mm/yyyy'
...
def process_generali_india_insurance_company(file_path, template_data, risk_code_data, cust_neft_data,
    ...
    try:
        ...
        # Remove empty rows to avoid empty dataframes
        data = data.dropna(how='all').reset_index(drop=True)
        data = data[data.apply(lambda row: row.count() > 6, axis=1)].reset_index(drop=True)
        print("Data after removing empty rows:")
        print(data.head(10))
...
def process_manipal_health_insurance_company(file_path, template_data, risk_code_data, cust_neft_data,
                                             table_3, table_4, table_5, subject, mappings):
    try:
        # Read the file based on its extension
        file_extension = os.path.splitext(file_path)[1].lower()
        ...
        for idx, section in enumerate(sections):
            ...
            # Set 'Entry No.' and other columns
            processed_df['Entry No.'] = range(1, len(processed_df) + 1)
            processed_df['Debtor Name'] = 'Manipal Cigna Health Insurance Company Limited (Prev. Cigna Ttk Health Insurance'
            processed_df['AccountType'] = "Customer"
            processed_df['AccountTypeDuplicate'] = processed_df['AccountType']
            processed_df['Nature of Transaction'] = "Brokerage Statement"
            processed_df['TDS Ledger'] = processed_df['Debtor Name']
        ...
        base_dir = (
            r'\\Mgd.mrshmc.com\ap_data\MBI2\Shared\Common - FPA\Common Controller'
            r'\Common folder AP & AR\Brokerage Statement Automation\Manipal Health Insurance Template Files'
        )
        excel_dir = os.path.join(base_dir, 'excel_file')
        csv_dir = os.path.join(base_dir, 'csv_file')
        ...
        print(f"Error processing Manipal health Insurance Company: {str(e)}")
        raise
...
def process_aditya_insurance_co(file_path, template_data, risk_code_data, cust_neft_data,
                                             table_3, table_4, table_5, subject, mappings):
    try:
        # Read the file based on its extension
        file_extension = os.path.splitext(file_path)[1].lower()
        ...
        for idx, section in enumerate(sections):
            ...
            # Set 'Entry No.' and other columns
            processed_df['Entry No.'] = range(1, len(processed_df) + 1)
            processed_df['Debtor Name'] = 'Aditya Birla Health Insurance Co.Ltd.'
            processed_df['AccountType'] = "Customer"
            processed_df['AccountTypeDuplicate'] = processed_df['AccountType']
            processed_df['Nature of Transaction'] = "Brokerage Statement"
            processed_df['TDS Ledger'] = processed_df['Debtor Name']
            ...
            processed_df['Narration'] = narration
            print(f"Narration set in processed data: {narration}")
            if gst_present:                # Create additional rows
                new_rows = pd.DataFrame({
                    'Entry No.': '',
                    'Debtor Name': processed_df['Debtor Name'].iloc[0],
                    'Nature of Transaction': ["GST Receipts", "Brokerage Statement"],
        ...
        base_dir = (
            r'\\Mgd.mrshmc.com\ap_data\MBI2\Shared\Common - FPA\Common Controller'
            r'\Common folder AP & AR\Brokerage Statement Automation\Aditya Birla Health Insurance Template Files'
        )
        excel_dir = os.path.join(base_dir, 'excel_file')
        csv_dir = os.path.join(base_dir, 'csv_file')
...
def process_star_india_diachi(file_path, template_data, risk_code_data, cust_neft_data,
                              table_3, table_4, table_5, subject, mappings):
    try:
        print("Starting the processing star india diachi data...")
        ...
        for idx, end_date in enumerate(unique_end_dates, start=1):
            ...
            base_dir = (
                r'\\Mgd.mrshmc.com\ap_data\MBI2\Shared\Common - FPA\Common Controller'
                r'\Common folder AP & AR\Brokerage Statement Automation\Star Union Dai-Ichi Life Insurance'
                r' Template Files'
            )
            excel_dir = os.path.join(base_dir, 'excel_file')
            csv_dir = os.path.join(base_dir, 'csv_file')
        ...
        print(f"Error processing Star Union Dai-Ichi Life Insurance Company Ltd data: {str(e)}")
        raise
...
def process_future_generalli_life_insurance(file_path, template_data, risk_code_data, cust_neft_data,
    ...
    try:
        ...
        for idx, end_date in enumerate(unique_end_dates, start=1):
            ...
            base_dir = (
                r'\\Mgd.mrshmc.com\ap_data\MBI2\Shared\Common - FPA\Common Controller'
                r'\Common folder AP & AR\Brokerage Statement Automation\Future Generali India Life Insurance'
                r' Template Files'
            )
            excel_dir = os.path.join(base_dir, 'excel_file')
            csv_dir = os.path.join(base_dir, 'csv_file')
...
def process_iffco_tokyo_insurer(file_path, template_data, risk_code_data, cust_neft_data,
                                table_3, table_4, table_5, subject, mappings):
    try:
        # Read the file based on its extension
        file_extension = os.path.splitext(file_path)[1].lower()
        ...
        for idx, section in enumerate(sections):
            ...
            # Set 'Entry No.' and other columns
            processed_df['Entry No.'] = range(1, len(processed_df) + 1)
            processed_df['Debtor Name'] = 'IFFCO TOKIO General Insurance Co. Ltd.'
            processed_df['AccountType'] = "Customer"
            processed_df['AccountTypeDuplicate'] = processed_df['AccountType']
            processed_df['Nature of Transaction'] = "Brokerage Statement"
            processed_df['TDS Ledger'] = processed_df['Debtor Name']
        ...
        base_dir = (
            r'\\Mgd.mrshmc.com\ap_data\MBI2\Shared\Common - FPA\Common Controller'
            r'\Common folder AP & AR\Brokerage Statement Automation\IFFCO TOKIO General Insurance Template Files'
        )
        excel_dir = os.path.join(base_dir, 'excel_file')
        csv_dir = os.path.join(base_dir, 'csv_file')
        ...
        print(f"Error processing IFFCO TOKIO General Insurance Co. Ltd.: {str(e)}")
        raise


def process_max_life_insurance(file_path, template_data, risk_code_data, cust_neft_data,
                                table_3, table_4, table_5, subject, mappings):
    try:
        # Read the file based on its extension
        file_extension = os.path.splitext(file_path)[1].lower()
        ...
        for idx, section in enumerate(sections):
            ...
            # Set 'Entry No.' and other columns
            processed_df['Entry No.'] = range(1, len(processed_df) + 1)
            processed_df['Debtor Name'] = 'Max Life Insurance Co. Ltd. ( Prev. Known As Max New York Life Insurance Co. Ltd'
            processed_df['AccountType'] = "Customer"
            processed_df['AccountTypeDuplicate'] = processed_df['AccountType']
            processed_df['Nature of Transaction'] = "Brokerage Statement"
            processed_df['TDS Ledger'] = processed_df['Debtor Name']
        ...
        base_dir = (
            r'\\Mgd.mrshmc.com\ap_data\MBI2\Shared\Common - FPA\Common Controller'
            r'\Common folder AP & AR\Brokerage Statement Automation\Max Life Insurance Template Files'
        )
        excel_dir = os.path.join(base_dir, 'excel_file')
        csv_dir = os.path.join(base_dir, 'csv_file')
...
def process_aditya_birla_sun_life(file_path, template_data, risk_code_data, cust_neft_data,
    ...
    try:
        ...
        base_dir = (
            r'\\Mgd.mrshmc.com\ap_data\MBI2\Shared\Common - FPA\Common Controller'
            r'\Common folder AP & AR\Brokerage Statement Automation\Aditya birla life Insurance Template Files'
        )
        excel_dir = os.path.join(base_dir, 'excel_file')
        csv_dir = os.path.join(base_dir, 'csv_file')
...
def process_sbi_life_insurance_co(file_path, template_data, risk_code_data, cust_neft_data,
                                  table_3, table_4, table_5, subject, mappings):
    try:
        print("Starting the processing SBI Life Insurance Company Limited data...")
        ...
        processed_df['Debtor Name'] = (
            processed_df['Debtor Name']
            if 'Debtor Name' in processed_df.columns
            else 'SBI Life Insurance Company Limited'  # Updated Debtor Name
        )
        # If Debtor Name exists, overwrite it with the insurer-specific name
        processed_df['Debtor Name'] = 'SBI Life Insurance Company Limited'
        processed_df['AccountType'] = "Customer"
        processed_df['AccountTypeDuplicate'] = processed_df['AccountType']
        processed_df['Nature of Transaction'] = "Brokerage Statement"
        processed_df['TDS Ledger'] = processed_df['Debtor Name']
        ...
        for idx, end_date in enumerate(unique_end_dates, start=1):
            ...
            # Set 'Debtor Name' as 'Insurer Name'
            df_section['Debtor Name'] = 'SBI Life Insurance Company Limited'  # Updated Debtor Name
            print("Set 'Debtor Name' as 'SBI Life Insurance Company Limited'.")
            ...
            if gst_present:
                narration = (
                    f"BNG NEFT DT-{date_col_formatted} rcvd towards brkg Rs."
                    f"{net_value:.2f} from {supplier_name_col} with GST 18%"
                )
            else:
                narration = (
            ...
            base_dir = (
                r'\\Mgd.mrshmc.com\ap_data\MBI2\Shared\Common - FPA\Common Controller'
                r'\Common folder AP & AR\Brokerage Statement Automation\SBI Life Insurance'
                r' Template Files'  # Updated folder path
            )
            excel_dir = os.path.join(base_dir, 'excel_file')
            csv_dir = os.path.join(base_dir, 'csv_file')
        ...
        print(f"Error processing SBI Life Insurance Company Limited data: {str(e)}")
        raise


def process_pramerica_life_insurance(file_path, template_data, risk_code_data, cust_neft_data,
                                  table_3, table_4, table_5, subject, mappings):
    try:
        # Read the file based on its extension
        file_extension = os.path.splitext(file_path)[1].lower()
        ...
        for idx, section in enumerate(sections):
            ...
            # Set 'Entry No.' and other default columns
            processed_df['Entry No.'] = range(1, len(processed_df) + 1)
            processed_df['Debtor Name'] = 'Pramerica Life Insurance Limited'
            processed_df['AccountType'] = "Customer"
            processed_df['AccountTypeDuplicate'] = processed_df['AccountType']
            processed_df['Nature of Transaction'] = "Brokerage Statement"
            processed_df['TDS Ledger'] = processed_df['Debtor Name']
        ...
        base_dir = (
            r'\\Mgd.mrshmc.com\ap_data\MBI2\Shared\Common - FPA\Common Controller'
            r'\Common folder AP & AR\Brokerage Statement Automation\Pramerica Life Insurance Template Files'
        )
        excel_dir = os.path.join(base_dir, 'excel_file')
        csv_dir = os.path.join(base_dir, 'csv_file')
        ...
        print(f"Error processing premerica Co. Ltd.: {str(e)}")
        raise
...
def process_pnb_metlife_insurance(file_path, template_data, risk_code_data, cust_neft_data,
                                  table_3, table_4, table_5, subject, mappings):
    try:
        # Read the file based on its extension
        file_extension = os.path.splitext(file_path)[1].lower()
        ...
        for idx, section in enumerate(sections):
            ...
            # Combine 'Income Category1' and 'Income Category2' into 'Income Category' based on the logic
            if 'Income Category1' in processed_df.columns and 'Income Category2' in processed_df.columns:
                print("Combining 'Income Category1' and 'Income Category2' into 'Income Category'")
                def combine_income_category(row):
                    income_cat1 = str(row['Income Category1']).strip()
                    income_cat2 = str(row['Income Category2']).strip()
                    if re.search(r'\d', income_cat1):  # Check if income_cat1 contains any digit
                        return income_cat2
                    else:
                        return income_cat1 + ' ' + income_cat2 if income_cat1 and income_cat2 else income_cat1 or income_cat2
                processed_df['Income Category'] = processed_df.apply(combine_income_category, axis=1)
            else:
                print("One or both of 'Income Category1' and 'Income Category2' not in processed data, setting 'Income Category' to empty.")
                processed_df['Income Category'] = ''

            # Set 'Entry No.' and other default columns
            processed_df['Entry No.'] = range(1, len(processed_df) + 1)
            processed_df['Debtor Name'] = 'PNB MetLife India Insurance Company Ltd (Pre. Met Life India Insurance Company P'
            processed_df['AccountType'] = "Customer"
            processed_df['AccountTypeDuplicate'] = processed_df['AccountType']
            processed_df['Nature of Transaction'] = "Brokerage Statement"
            processed_df['TDS Ledger'] = processed_df['Debtor Name']
        ...
        base_dir = (
            r'\\Mgd.mrshmc.com\ap_data\MBI2\Shared\Common - FPA\Common Controller'
            r'\Common folder AP & AR\Brokerage Statement Automation\PNB MetLife Insurance Template Files'
        )
        excel_dir = os.path.join(base_dir, 'excel_file')
        csv_dir = os.path.join(base_dir, 'csv_file')
        ...
        print(f"Error processing PNB MetLife India Insurance Company Ltd: {str(e)}")
        raise
